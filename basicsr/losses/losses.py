...
class TripletLoss(nn.Module):
    def __init__(self, loss_weight=1.0, reduction='mean', margin=1.0):
        super(TripletLoss, self).__init__()
        self.loss_weight = loss_weight
        self.criterion = torch.nn.TripletMarginLoss(margin=margin, reduction=reduction)

    def forward(self, anchor, positive, negative):
        return self.loss_weight * self.criterion(anchor, positive, negative)
...
from .loss_util import normalize_activation, get_state_dict
...
class LPIPSLoss(nn.Module):
    def __init__(self, net_type = 'vgg', version = '0.1', lpips_weight=1.0):
        ...
        if net_type == 'vgg':
            ...
            raise NotImplementedError('error net type for LPIPS')
